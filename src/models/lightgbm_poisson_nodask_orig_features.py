    ...
    def __init__(
        ...
        if ".parquet" not in s3_path:
            try:
                ...
                self.full_dataset = pd.read_parquet(
                    s3_path, engine='pyarrow'
                ).sample(frac=self.frac, random_state=42)
    ...
    def gridsearch_wfv(self, params):
        ...
        for params_comb_counter, params_comb_dict in enumerate(
            chain.from_iterable(
                ...
                for d in [{k: [v[i]] if k in ('max_depth', 'num_leaves') else v
                for k, v in params.items()} for i in range(len(params['max_depth']))]
            ),
            1,
        ):
        # for params_comb_counter, params_comb_dict in enumerate(
        ...
        for train_counter, m in enumerate(range(n_val_sets), 1):
            ...
            yield (
                ...
                self.full_dataset[
                    ...
                    (self.full_dataset.index <= end_date + relativedelta(months=self.n_months_in_val_set, day=31))
                ],
