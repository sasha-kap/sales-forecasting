...
params = {
    ...
    "dispersion": [-1.5, -1.0, -0.5, -0.2, -0.1, 0.01, 0.05],
    # "dispersion": [0.1, 0.5, 1.0, 1.5, 2.0],
}
...
class LightGBMDaskLocal:
    ...
    def truncated_negbin(self, y_true, y_pred):
        ...
        phi = self.params_comb_dict["dispersion"]
        loss = (
            (y_true + 1 / phi) * np.log(1 + phi * np.exp(y_pred))
            + np.log(1 - (phi * np.exp(y_pred) + 1) ** (-1 / phi))
            - y_true * np.log(phi * np.exp(y_pred))
        )
        grad = (
            (np.exp(y_pred) * phi * (y_true + 1 / phi)) / (np.exp(y_pred) * phi + 1)
            + np.exp(y_pred)
            * (np.exp(y_pred) * phi + 1) ** (-(phi + 1) / phi)
            / (1 - (np.exp(y_pred) * phi + 1) ** (-1 / phi))
            - y_true
        )
        # hess = d^2/dx^2((y + 1/ϕ) log(1 + ϕ exp(x)) + log(1 - (ϕ exp(x) + 1)^(-1/ϕ)) -
        # y log(ϕ exp(x))) = ((e^x ϕ)/(e^x ϕ + 1) - (e^(2 x) ϕ^2)/(e^x ϕ + 1)^2) (y + 1/ϕ) +
        # (e^x (e^x ϕ + 1)^(-1/ϕ - 1))/(1 - (e^x ϕ + 1)^(-1/ϕ)) +
        # (e^(2 x) (-1/ϕ - 1) ϕ (e^x ϕ + 1)^(-1/ϕ - 2))/(1 - (e^x ϕ + 1)^(-1/ϕ)) -
        # (e^(2 x) (e^x ϕ + 1)^(-2/ϕ - 2))/(1 - (e^x ϕ + 1)^(-1/ϕ))^2
        hess = (
            (
                (np.exp(y_pred) * phi) / (np.exp(y_pred) * phi + 1)
                - (np.exp(2 * y_pred) * phi ** 2) / (np.exp(y_pred) * phi + 1) ** 2
            )
            * (y_true + 1 / phi)
            + (np.exp(y_pred) * (np.exp(y_pred) * phi + 1) ** (-1 / phi - 1))
            / (1 - (np.exp(y_pred) * phi + 1) ** (-1 / phi))
            + (
                np.exp(2 * y_pred)
                * (-1 / phi - 1)
                * phi
                * (np.exp(y_pred) * phi + 1) ** (-1 / phi - 2)
            )
            / (1 - (np.exp(y_pred) * phi + 1) ** (-1 / phi))
            - (np.exp(2 * y_pred) * (np.exp(y_pred) * phi + 1) ** (-2 / phi - 2))
            / (1 - (np.exp(y_pred) * phi + 1) ** (-1 / phi)) ** 2
        )
        return grad, hess
        ...
        for params_comb_dict in (
            ...
            try:
                self.model = lgb.LGBMRegressor(
                    # client=self.client,
                    objective=self.truncated_negbin,
                    random_state=42,
                    silent=False,
                    # tree_learner="data",
                    # force_row_wise=True,
    ...
    def fit(self, train):
        try:
            ...
            logging.exception(
                "Exception occurred while fitting model on train data during walk-forward "
                "validation with parameter values equal "
                f"to {pformat(self.params_comb_dict)}"
            )
            return self
