        ...
        try:
            ...
            try:
                ...
                callback = callbacks.EarlyStopping(
                    monitor="val_rmse_metric_for_classes",
                    patience=10,
            ...
            # encode target in multilabel format with one fewer labels than possible values of target
            # references:
            ...
            ohe = OneHotEncoder(
                categories=[np.arange(1, 6)], sparse=False, dtype="int8"
            )
            ohe_train_y = ohe.fit_transform(
                train["sid_shop_item_qty_sold_day"].to_numpy().reshape(-1, 1)
            )
            multilabel_train_y = np.apply_along_axis(self.multilabel_encode, 1, ohe_train_y)
            multilabel_train_y = np.delete(
                multilabel_train_y, 0, 1
            )  # deletes first column to get K-1 class encoding
            ohe_test_y = ohe.transform(
                test["sid_shop_item_qty_sold_day"].to_numpy().reshape(-1, 1)
            )
            multilabel_test_y = np.apply_along_axis(self.multilabel_encode, 1, ohe_test_y)
            multilabel_test_y = np.delete(
                multilabel_test_y, 0, 1
            )  # deletes first column to get K-1 class encoding
            ...
            history = self.model.fit(
                ...
                y=multilabel_train_y,
                validation_data=(self.test_X, multilabel_test_y,),
                epochs=50,
                batch_size=64,
                shuffle=True,
                callbacks=[callback],
